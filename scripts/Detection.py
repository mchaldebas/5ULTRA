...
UTR_FILE_PATH = './Databases/5UTRs.tsv'
UORF_FILE_PATH = './Databases/uORFs.tsv'
...
def process_variant(variant, utrs_by_chromosome, uorfs_by_transcript):
    ...
    for UTR in chromosome_utrs:
        ...
        if UTR[0] != CHR or not (int(UTR[1]) < POS < int(UTR[2])):
            continue
        # check if the variant is in an exon
        status = 'out'
...
def main():
    ...
    with open(OUTPUT_FILE_PATH, 'w') as f:
        ...
        fields = variants[0] + ['CSQ', 'translation'] + UTRs[1:12] + UTRs[14:] + uORFs[1:3] + [uORFs[4]] + uORFs[17:-4] + uORFs[-3:]
